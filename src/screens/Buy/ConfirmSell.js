...
const ConfirmSell = props => {
  ...
  const {confirmSell, token} = useSelector(store => store.appReducer);
  ...
  const SendData = async () => {
    ...
    data.append('currency', confirmSell.data.finance_param.currency);
    data.append('currency_name', confirmSell.data.currency_name);
    data.append('guid', confirmSell.data.finance_param.guid);
    ...
    appAxios
      ...
      .then(({data, ...res}) => {
        if (data.result === 1) {
          dispatch(ChekData(confirmSell.data.finance_param));

          setTimeout(() => {
            props.navigation.navigate('SellCheck');
            setloading(false);
          }, 2000);
        } else {
          setloading(false);
    ...
    <View style={{paddingHorizontal: 28}}>
      ...
      <View
        ...
        <Text style={s.second_text}>
          {confirmSell.data.finance_param.sum + ' ' + 'USDT'}
        </Text>
        {/* --------------------------------------------------------------- */}
        <Text style={s.first_text}>Сумма к зачислению </Text>
        <Text style={s.second_text}>
          {confirmSell.data.finance_param?.credit +
            ' ' +
            confirmSell.data.finance_param.currency}
        </Text>
        {/* -------------------------commission------------------------- */}
        <Text
          style={[
            ...
            {
              display:
                confirmSell.data.finance_param?.commission === ''
                  ? 'none'
                  : 'flex',
            },
          ]}>
        ...
        <Text
          style={[
            ...
            {
              display:
                confirmSell.data.finance_param?.gateway_fees === ''
                  ? 'none'
                  : 'flex',
            },
          ]}>
        ...
        <Text
          style={[
            ...
            {
              display:
                confirmSell.data.finance_param?.extra_fees === ''
                  ? 'none'
                  : 'flex',
            },
          ]}>
        ...
        <Text
          ...
          {confirmSell.data.finance_param?.memo}
        </Text>
      </View>
      {loading ? (
